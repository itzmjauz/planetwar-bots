...
def AddToSheet(results, sheets, coords, columns):
    ...
    for data in results:
        ...
        if (last_wb!=sheets[data[2]]):
            ...
            ws = wb.active
        column = columns[sys.argv[1].split("_")[2]]+int(sys.argv[1].split(".")[0][-1])
...
games = sorted([extract(i) for i in range(0,len(file)-2,3)], key=lambda x: x[3])
...
AddToSheet(games, sheets, rows, columns)
